...
function EditProductForm(props) {
  const [product, setProduct] = useState({
    ...
    description: '',
    variants: [],
    mainImage: null, // Added mainImage state
    additionalImages: [],
  });
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://192.168.1.121:3030/product/${id}`)
      .then((response) => {
        const productData = response.data;
        // Set the product state with the received data
        setProduct({
          ...
          mainImage: null,
          additionalImages: productData.productImages,
        });
        // Set the selected category ID
        setSelectedCategoryId(productData.categories[0].id);
      })
      .catch((error) => {
  ...
  const handleAddOption = (variantIndex) => {
    ...
    updatedVariants[variantIndex].VariationOptions.push({ name: '', price: '' });
  ...
  const onDropAdditionalImages = (acceptedFiles) => {
    setProduct({
      ...product,
      additionalImages: [...product.additionalImages, ...acceptedFiles],
    });
  };
  ...
  const handleSubmit = () => {
    ...
    const productData = {
      ...
      images: product.additionalImages.map((image) => ({ img: image.img })),
      img: product.mainImage ? product.mainImage.path : null,
    ...
    const id = props.params.id;
    axios
      .put(`http://192.168.1.121:3030/product/${id}`, productData)
      .then((response) => {
        console.log('Product updated:', response.data);
        // Redirect or perform any necessary action after a successful update
      })
      .catch((error) => {
            ...
            <div key={variantIndex} className="variant-form">
              <Form.Group>
                ...
                <Form.Control
                  ...
                  onChange={(e) => {
                    const updatedVariants = [...product.variants];
                    updatedVariants[variantIndex].title = e.target.value;
                    setProduct({ ...product, variants: updatedVariants });
                  }}
                />
              </Form.Group>
              ...
              <Form.Group>
                ...
                {variant.VariationOptions.map((option, optionIndex) => (
                  <div key={optionIndex} className="option-form">
                    <Form.Group>
                      ...
                      <Form.Control
                        ...
                        onChange={(e) => {
                          const updatedVariants = [...product.variants];
                          updatedVariants[variantIndex].VariationOptions[optionIndex].name =
                            e.target.value;
                          setProduct({ ...product, variants: updatedVariants });
                        }}
                      />
                    ...
                    <Form.Group>
                      ...
                      <Form.Control
                        ...
                        onChange={(e) => {
                          const updatedVariants = [...product.variants];
                          updatedVariants[variantIndex].VariationOptions[optionIndex].price =
                            e.target.value;
                          setProduct({ ...product, variants: updatedVariants });
                        }}
                      />
